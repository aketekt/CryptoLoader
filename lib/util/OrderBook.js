...
OrderBook.prototype.start = function() {
    ...
    // orderTrade feed parameters
    let params = {
        currencyPair: this._currencyPair
    };

    // Start listening to the push API
    this._wrapperPush.orderTrade(params, (err, response) => {
...
OrderBook.prototype._bufferUpdate = function(update) {
    ...
    if (this._statusSync == OrderBook.SYNC_STATUS_DESYNCHRONIZED) {
        ...
        this._wrapperPublic.returnOrderBook(params, (err, response) => {
            // Stop if an error occurred
            if (err) {
                this.stop(err);
        ...
        if (update.seq == this._lastHandledSeq || update.seq == this._lastHandledSeq + 1) {
            ...
            let oldestSeq = this._updateBuffer[oldestIdx].seq;
            let newestSeq = this._updateBuffer[newestIdx].seq;

            // Handle as many consecutive updates as possible currently in the buffer
            for (let seq = oldestSeq; seq <= newestSeq; seq++) {
                // Was this seq found?
                let found = false;
...
OrderBook.prototype._handleUpdate = function(update) {
    ...
    switch (update.updateType) {
        case "orderBookModify": {
            ...
            let updateRate = parseFloat(update.rate);
            let updateAmount = parseFloat(update.amount);
